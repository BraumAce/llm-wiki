...
12. **自主迭代要把训练信号与上线判断隔离**：训练集可供 Agent 分析 case 和扣分原因，验证集只返回分数；每轮 challenger 必须和未过拟合的 champion 在验证集上比较总分、指标、关键得分行、badcase 模式和过滤比例，胜出才晋级。这样既避免 reward hacking，也保留了策略退步时的回退基线。
