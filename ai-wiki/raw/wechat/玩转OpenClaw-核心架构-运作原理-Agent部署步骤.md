...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_gif/j3gficicyOvasVeMDmWoZ2zyN8iaSc6XWYj79H3xfgvsqK9TDxOBlcUa6W0EE5KBdxacd2Ql6QBmuhBJKIUS4PSZQ/640?wx_fmt=gif&from=appmsg#imgIndex=0)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz907nzSk022Tl19yGnPPqWkSQy7EM06BEHglon20JkF0vaOCYziaMLXCgmvks8ldy7q6bJqHRL1dDPR2DwXvX6ibmPAc55YM4GvQS4/640?wx_fmt=png&from=appmsg#imgIndex=1)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_jpg/KVER9adz905ttAIn295hTx09BxfU6oPIZTI0BgOaWCn14Ty8kHdsxzQeaIPWpworGaJnpfz6qF3X1tHsHS15nib1iax05Un4arbI4icicI2MN5k/640?wx_fmt=jpeg&from=appmsg#imgIndex=2)
...
exportasyncfunction loadWorkspaceBootstrapFiles(dir: string): Promise<WorkspaceBootstrapFile[]> {
const entries = [
...
![图片](https://mmbiz.qpic.cn/mmbiz_jpg/KVER9adz905RtF82Yqfic7Fm2YAsBhGCCAsuvB2rEbekLfPj3WHGIKHq3NInLd76W31Y1fXkTOXwJmGldviaN7STX9NLTckXVUnZZgghfWfsk/640?wx_fmt=jpeg&from=appmsg#imgIndex=3)

#### （三）单Agent架构会出现什么问题？

![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905tw7ltBcicVok7yfD2PbksDURCiaa883D6OD6mCOmLhnXe48vgM9iaWGibNEhOPRLvpZwgS6qcP9BuDyLLdlkpWPNUpbfKB5z5qkw/640?wx_fmt=png&from=appmsg#imgIndex=4)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz906fVoaaEGMkFuEwTDZia04Elw07JCg0B2TadxyvCPicsnjs37RodD1a5HibI1Rh0ecMewIavNdian3BZo3Au0PKZZcZQ9S5InuVYyo/640?wx_fmt=png&from=appmsg#imgIndex=5)
...
简单来说，当Agent之间的通信，需要被写入各自的memory时，就建议使用 sessions_send 的方式。

拿公司举例：给同事发消息，他在自己工作的上下文中处理，这种情况就很适合使用sessions_send。

（2）sessions_spawn: 在独立环境中运行任务

sessions_spawn 采用SubAgent的方式，通过调起子Agent+传入内容的方式，让其完成交付。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz9050KBtepAIes75s7puic4wyOGbBxnV5SZRV25yxRRB3Np2VWNaeUcIUkcjRLEJYBtOooMdjZKDVqGQa5MFSW5Ib8DibliaYTSy20g/640?wx_fmt=png&from=appmsg#imgIndex=6)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906ShqxmEWxgicvrCrPYPvKX7mkFqHtoyqu0mIGw7u9CXp0MucZ4AYicjU3qwgaSPBc7ZOCfcsr0pq6qojMKRkFN5xpibBOs9qrJl8/640?wx_fmt=png&from=appmsg#imgIndex=7)

如图所示，ceo → iostutor的会话，已经写入iostutor的session中，Agent之间可以互相进行通信了。

（2）配置 sessions_spawn

配置 sessions_spawn 也不负责，比如我们需要让 iostutor 成为 ceo 的SubAgent，那么我们就这么配置：
...
问题：当我们配置了A和B Agent互为SubAgent，且允许AgentToAgent时，Agent是怎么决定调用sessions_send 还是 sessions_spawn 的？

这取决于LLM自己的判断，比如我们配置了 ceo 和 iostutor 允许sessions_send ，也配置了 ceo→iostutor sessions_spawn。
...
这是因为我们上面的配置，让Agent之间的通信是用session来记忆，它能记住我们会话里说的Agent是工程里的角色（因为我们给它发送了这么一句话：”使用 sessions_send 工具，参数：sessionKey="agent:ceo:main", message="测试 agent-to-agent 通信", timeoutSeconds=60” ）。

但当短期记忆遗忘后，它就忘记了这个指令，所以为了解决这个问题，我们需要在 workspace/agent_xx/AGENTS.md 进行配置，明确告诉它不同Agent分别是谁，such as：
...
A Agent和B Agent 使用 sessions_send 通话后，间隔6小时后，A和B还能基于6小时以前的session继续沟通吗？会新开启一个session吗？还是复用之前的session呢？

结果是：会复用之前的session。

session_send 不创建新的session，它只向已存的session发消息。

6小时前 A 和 B 通过 sessions_send 通话时：
...
如上文所述，建议Agent同时配置 sessions_send 和 sessions_spawn。
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz906dC662uKuvjSibiaqz8Wia86mY6Ush9pex21Vws3AZq9uKp5ia4fxIY82H8dAJcsqHWYgn2NhTVX0JZquWGkrTvW6OnaibY8pnKib3A/640?wx_fmt=png&from=appmsg#imgIndex=8)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/KVER9adz904FDic1HFc43MECpKTBddPT4FrYbp9ibz5oVjqicA2zqlWibXzu3MZdzJabLpZKpfyc6F4n6NGgZQypc2Ydh92xrWZ9wGyzCxSibFlo/640?wx_fmt=png&from=appmsg#imgIndex=9)
...
![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905cYmx92XMria1zrvBGTQbUKjXwiawqXTLNP22XDXPInR4st6gRfZwia72aiaAzlfbe9bjASSUicrwz4oUDV0xXkJu1HJqIJ0ofnvDM/640?wx_fmt=png&from=appmsg#imgIndex=10)
...
AI 时代消息太多，推荐 [https://huggingface.co/papers](https://huggingface.co/papers) 的daily_paper，可以通过Agent进行每日论文的抓取，让它快速提炼论文要点，让我们从源头了解AI的前言信息。

注：Agent直接获取 [https://huggingface.co/papers](https://huggingface.co/papers) 容易失败，可以考虑jina.ai这个工具。

![图片](https://mmbiz.qpic.cn/mmbiz_png/KVER9adz905gXF6HLRwZHDLZWcLJnK5VuVCcGUOOnY7eG3jJunrK1gqNBMxy0ia3eWT8m6U0RpDx5aSdQK9qhPVI3HVCx9D6jDibDUN4PcbhU/640?wx_fmt=png&from=appmsg#imgIndex=11)
...
通过在 Workspace/Agent_xx/Memory/xxx.md 目录下配置学习资料，我们可以让Agent成为我们垂直领域的专属tutor，借助OpenClaw的能力，用最快的方式实现一套RAG tutor。
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_jpg/KVER9adz905alZ1WdgmVX7GzY3WicMz3Yhwz80GzOeBgnwViaC0C2hicWV4Phkic2TyEib7UazXd5zBPD9PnicGMMNBVLLQ3eGw46kUOgDucAbQ80/640?wx_fmt=jpeg&from=appmsg#imgIndex=12)
...
![图片](https://mmbiz.qpic.cn/sz_mmbiz_png/j3gficicyOvasVeMDmWoZ2zyN8iaSc6XWYj5q5PQEOc5ibURPb03vnRibrxC3UR8xzdyATfiawTYRV2vJvBnAIcE1FeQ/640?wx_fmt=png&from=appmsg#imgIndex=14)
