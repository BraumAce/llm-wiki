...
![](https://mmbiz.qpic.cn/sz_mmbiz_jpg/j7RlD5l5q1wqnnPehjOibJB4MiaVTKzPn0NI3P0jJhpVcxyVF76R4v1wnuQq7gsSa3a42j5cicRZzHG4636qBs6RnYtGrLcb7UhMJcdgiapeVyU/640?wx_fmt=jpeg&from=appmsg)
...
自动触发：靠的是description 字段。当用户的意图和 Skill 描述语义匹配时，Agent 会自己判断"这个任务我有现成的专业流程可以用"，然后主动加载对应的 Skill。整个过程用户无感，就像一个老员工听到需求就自动翻出了对应的 SOP。
...
SKILL.md 应控制在500行以内，500行文本等于2000～3000 token，是单个 Skill 激活后比较合理的上下文开销。
...
![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1wCicxZib6mj1rTs1P12xGnTGpJRHjEzjV8j1sgaNL4wt8nsdcrbwEzoKc2YYQKKJpEUfxQRKnslm7gzQZSGc5mvRBGcuKVG7fWg/640?wx_fmt=png&from=appmsg)
...
最佳实践：trade-ab-skill 的用户偏好持久化：成功操作后，将关键参数写入
```
user-prefs.json
```
，下次执行时自动注入：
...
「帮我用 skill-creator 生成一个名为order-query的 Skill」
...
「把 xxx 的详细流程整理成references/workflow.md，并在 SKILL.md 中加上引用。」

Step 4：打包验证

告诉 AI：

「用 skill-creator 打包验证order-query，检查格式是否合规。」
...
「帮我把order-query初始化为 git 仓库，关联远端地址，提交所有文件并推送到 main 分支。」

Step 6：后续迭代更新

修改 Skill 内容后，告诉 AI：

「帮我提交order-query的最新改动，commit 信息是 xxx，然后推送。」
