...
author: "阿里云开发者"
fetched_at: "2026-07-01T22:35:17+08:00"
fetcher: "direct-wechat-html"
publish_time: ""
---
...
![](https://mmbiz.qpic.cn/mmbiz_jpg/j7RlD5l5q1yAJ3L6nf8ic8yq7IRQfa589sA87Auic0XTv89O7h8fhBPcCzs6avMuBp4fibUXV6uKbDHogQvBXd8jvUYzGh7cQgld6hyxae9Sfo/640?wx_fmt=jpeg&from=appmsg)
...
![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1yE2PdflA8pIoX611qEc5U9lHFDE985OTz8wTOraaWf4j1feFv9l8hAnd9XOGomRxUZWs00b0HA3pNexcdFYmkEpyHQqKZ0PzU/640?wx_fmt=png&from=appmsg)
...
一个会话里它表现得像高级架构师，下一个会话像刚入职的实习生，因为**它压根不记得上次干了什么**。
...
![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1xjuT7kGkwRLk0EI5uNzsZwWcXYsTHupicqPuSINpDx0gvsZCtsrm3UBxAeCGhHXM7UAtwbAlwwylDjOCqV9icMcNvRgXSe59DGo/640?wx_fmt=png&from=appmsg)
...
把 Prompt、Context、Harness 三代范式落到系统层面，本质是一张"围绕 Model 展开的运行时全景图"。你可以把下面这张图当作后文所有铁律、模式、标杆案例的**物理地图**——每一条最佳实践，都对应图中的某一个位置。

![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1yuiaBAr72btVlia5tShI1w4UkuO35HNr1lQGrBfKKrflFVA2eGVdLpftQ1EEuGJAmzicviaXCgXm1vjnSG15DuM7X4GeAfOVGPx0g/640?wx_fmt=png&from=appmsg)
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1zI5gkgu5VFRk7w66fZH8u7icYficp9dRdTRQs2ufjrRHP7XSUY33HArZ0xzWEzC5iblk2WQ4U0ME8icrkibEs0cEnqdstNh9hTJJxM/640?wx_fmt=png&from=appmsg)

↑ 四张卡片就是后文所有工程模式的"DNA"：上下文要少、Agent 要专、状态要落盘、约束要可执行。每一条都和工程师的本能反着来——而这正是它们值钱的原因。

读了几十篇文章、踩了三个月坑之后，我把 Harness 的精髓提炼为四条**反直觉**的铁律。之所以"反直觉"，是因为它们都和工程师的本能背道而驰。
...
业内多个团队的实测都指向同一个方向：长上下文窗口在**实际使用比例超过一定阈值之后**，模型的准确率会显著下降——具体阈值因模型、任务、Prompt 结构差异很大，并不存在一个放之四海皆准的"百分比"。
...
AGENTS.md
```
里每一条规则，背后都对应一个真实失败案例；而能写成静态检查 / CI 拦截的规则，绝不让它停留在自然语言里——因为**自然语言可以被模型"创造性解读"，代码不能**[1]。
...
Harness Engineering 不是 Context Engineering 的对立面，而是它的**工业化升级**。同样是"给模型对的信息"，工程化版本要做四件事：
...
两个 Agent**不共享 Context Window**，只通过 Workspace 里的
```
plan.md
```
...
Agent 选错工具的最大原因，不是工具太多，而是**工具签名写得像一坨胶水**。成熟团队的做法：
...
主 Agent 只接收 Sub-Agent 的**结构化输出**，不接收它的中间思考。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1yj73ibAnuvMvPUk89vK4RkPdgzMB2l8kJR6WdeUSX0pMFe3Tt4QahibucuhDkYgxHkpw3UzO1NZia3PibgLtJwME6wcYbXic8HiawW4/640?wx_fmt=png&from=appmsg)
...
理论铁律说了一大堆，接下来上一个**正在生产环境跑**的真实案例。这是我们团队在钉钉企业级 Agent「悟空」上落地的**AI 招聘 Agent 系统**——它每天处理上千份简历、自动约面试、跟踪候选人状态，已经稳定运行数月。
...
```
招聘工作台RPA跑到一半，Agent"顺手"去回复了候选人聊天 → 上下文炸了
```

```
10+ 个工具里挑错工具的概率显著偏高（明明该调 query_calendar，调成了 query_db）
```

```
Prompt600行写到第400行，模型已经忘了开头说"不要主动发 Offer"
```
...
——这正是**铁律二（专才 > 通才）**和**铁律一（上下文越少越好）**的双重违反现场。

![](https://mmbiz.qpic.cn/mmbiz_png/j7RlD5l5q1wa6IwjnLDwe4u6WpiceTMOZhCGRWBicy9Bic2GYEddXWEg6Jwv5lP2lk4H81fj653J6cN2pgzyuy54vpZVibqN1jJRuC9sQl7rNk0/640?wx_fmt=png&from=appmsg)
...
最关键的一点：**悟空不直接调 n 个工具**。它只调"派单"这一个动作——把任务连同**精挑过的 3-5 个工具**派给专才 Agent。这就把"50 选 1"的选择题，拆成了三道"5 选 1"。
...
人岗匹配 Agent 在招聘工作台里做 RPA 操作时，最早会出现"做到一半上下文被打断、状态对不上"的事故。后来我们引入**强制性的事务文件**：
...
招聘沟通 Agent 是唯一一个"会主动给真人发消息"的 Agent——这就意味着它的错误是**对外暴露的**。我们给它套了三层硬护栏：
...
下面是基于当前趋势的一组**判断**，不是预言。每一条都标注了可证伪条件——这是资深工程判断和"预言式断言"的分水岭。

**趋势一：从"Prompt 工程师"到"Agent 工程师"**

招聘市场上，Prompt 工程师的岗位正在快速被"Agent 工程师 / Harness 工程师 / AI 系统工程师"这类岗位取代。区别在于：

Prompt 工程师卷的是**话术**，可被替代性强。

Harness 工程师卷的是**系统**——上下文设计、工具签名、反馈回路、状态管理——这些就是软件工程的核心能力。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1zwgiaSaHOo5uhrABKRD6maicghjSVibSdPZwtbHVlicqkffuia6bSzHsRlwPyFAhXt02FSY4VnGIM8XwtibFRKmhSmxQF5hDhLYYSZ8/640?wx_fmt=png&from=appmsg)
...
现在，真正拉开差距的——是**谁的缰绳更顺手、马鞍更舒服、护栏更结实**。

模型还在进步，但**单靠等待更强的模型已经不够了**。工程能力的差距，正在成为新的分水岭。**真正的护城河，是你为这匹野马造的那条高速公路**——它决定了你能跑多远、多稳、多快。
...
![](https://mmbiz.qpic.cn/sz_mmbiz_png/j7RlD5l5q1xQoXwoB2Pbh0ZwLcy5OGICxEWekJr6s7FrPfXHmFdibz8Ro6Zsl9M9DOQ8SnhUPGL8tdp2gkTzeZWk4KF7icemkUJLpGCMzOenc/640?wx_fmt=png&from=appmsg)
