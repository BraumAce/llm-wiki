...
# 2) 健康检查
bash .claude/skills/llm-wiki-skill/scripts/lint.sh
bash .claude/skills/llm-wiki-skill/scripts/status.sh
...
```bash
export DEPLOY_HOST=root@your.server.ip
export DEPLOY_PATH=/var/www/llm-wiki
export DEPLOY_DOMAIN=your-domain.com
```

服务器侧 nginx + Let's Encrypt 一次性准备见 [deployment-nginx-vps.md](.claude/skills/quartz-wiki/references/deployment-nginx-vps.md)。
...
- [ ] GitHub Actions 自动部署（main push 触发 rsync）
